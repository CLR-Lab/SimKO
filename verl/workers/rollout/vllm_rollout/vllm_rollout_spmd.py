...
class vLLMRollout(BaseRollout):
    ...
    # def find_think_spans(self,token_ids):
    #     spans = []
    #     i = 0
    #     while i < len(token_ids):
    #     # 匹配 <think>
    #     if token_ids[i:i+len(self.think_start_ids)] == self.think_start_ids:
    #         start = i
    #         i += len(self.think_start_ids)
    #         # 匹配 </think>
    #         while i < len(token_ids):
    #             if token_ids[i:i+len(self.think_end_ids)] == self.think_end_ids:
    #                 end = i + len(self.think_end_ids) - 1
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        # response_think_mask = torch.zeros_like(response_attention_mask)


        # for b in range(batch_size):
        #     resp = response[b].tolist()
        #     spans = find_think_spans(resp, self.think_start_ids, self.think_end_ids)
        #     for start, end in spans:
        #         # response 在 seq 里的起始下标是 prompt_len
        #         response_think_mask[b, start : end + 1] = 1
        # think_mask = torch.cat((think_mask, response_think_mask), dim=-1)
        ...
        #     think_response=self.tokenizer.decode(response[i]*think_mask[i])
        #     print(think_response)
        #     print("--------------------------------")
        #     response_decode=self.tokenizer.decode(response[i])
